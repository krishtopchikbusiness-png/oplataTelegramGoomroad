...
def admin_request_result_text(plan_code: str, username: str | None, tg_user_id: int, confirmed: bool) -> str:
    plan = PLANS[plan_code]
    username_text = username if username else "без username"
    status_line = "✅ Оплата подтверждена" if confirmed else "❌ Оплата не подтверждена"
    return (
        "📩 Новая заявка на проверку оплаты\n\n"
        f"Тариф: {plan['name']}\n"
        f"Сумма: {plan['amount']}\n"
