...
async def _create_fresh_join_message(
    *,
    ...
    existing_sub = await db.get_subscription(tg_user_id)
    if existing_sub and existing_sub.invite_link:
        try:
            await context.bot.revoke_chat_invite_link(settings.group_id, existing_sub.invite_link)
...
async def join_request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if join_request.invite_link:
            used_invite = join_request.invite_link.invite_link
    except AttributeError:
        used_invite = None

    # Пускаем только активную подписку и только по актуальной ссылке, выданной этому юзеру
    if (
        not sub
        or sub.status != "active"
        or sub.access_until <= now_dt
        or not sub.invite_link
        or used_invite != sub.invite_link
    ):
        try:
