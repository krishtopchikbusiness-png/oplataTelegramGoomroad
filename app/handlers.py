...
async def pay_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    pending = await db.get_pending_payment_request_for_user(user.id)
    if pending:
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not request:
        await query.answer("Заявка не найдена.", show_alert=False)
        return
    ...
    if action == "reject":
        ...
        try:
            await query.edit_message_text(
                text=admin_request_result_text(request.plan_code, request.tg_username, request.tg_user_id, confirmed=False)
            )
    ...
    try:
        await query.edit_message_text(
            text=admin_request_result_text(request.plan_code, request.tg_username, request.tg_user_id, confirmed=True)
        )
    except TelegramError:
        try:
            await query.edit_message_reply_markup(reply_markup=None)
