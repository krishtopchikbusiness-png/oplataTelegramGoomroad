...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_message:
        return
    await update.effective_message.reply_text(text=start_text(), reply_markup=start_keyboard())
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "back:start":
        await query.answer()
        await query.edit_message_text(text=start_text(), reply_markup=start_keyboard())
        return
...
async def verify_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(chat_id=chat.id, text=payment_wait_text())
...
async def on_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    opened_msg = await context.bot.send_message(
        chat_id=subscription.last_chat_id,
        text=channel_opened_text(),
        reply_markup=open_channel_keyboard(settings.channel_url),
    )
    context.job_queue.run_once(
        callback=delete_message_later,
        when=180,
        data={"chat_id": subscription.last_chat_id, "message_id": opened_msg.message_id},
        name=f"delete_opened_{subscription.tg_user_id}_{opened_msg.message_id}",
    )
